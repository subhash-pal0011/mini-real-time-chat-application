...
//   // 🟢 API: fetch chat profiles (users you’ve chatted with)
...
//   // Map chatUser to show other participant only (exclude login user)
...
//       const participant = otherParticipants[0]; // first other participant
//       return {
//         ...participant,
...
//     })
//     .filter(Boolean)
//     .sort((a, b) => new Date(b.lastUpdated) - new Date(a.lastUpdated)); // latest first
...
//             scale: 1.15,
//             boxShadow:
//               "0 0 20px rgba(236,72,153,0.10), 0 0 40px rgba(214, 219, 226,0.6)",
//           }}
...
const LeftContainer = () => {
  ...
  useEffect(() => { fetchChaters(); }, []);

  const handleBackClick = () => { setSearchUser([]); setValue("text", ""); };
  const handleSelectChat = (chat) => { setSelectedConversation(chat); clearUnread(chat.conversationId); handleBackClick(); };
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-b from-white to-gray-50 border-r border-gray-200 shadow-lg">
      ...
      <div className="p-3 border-b border-gray-100 flex items-center gap-2 bg-gray-50">
        <div className="relative flex-1">
          <input type="text" placeholder="Search..." {...register("text")} className="w-full pl-10 pr-10 py-3 rounded-full bg-white border border-gray-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm placeholder-gray-400 transition-all duration-300 cursor-pointer" />
          <IoSearch className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-blue-500 text-lg" />
          {loading && <AiOutlineLoading3Quarters className="absolute right-3 top-1/2 transform -translate-y-1/2 text-blue-500 text-lg animate-spin" />}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
        {searchUser.length ? (
          <div>
            <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-200">
              <GrLinkPrevious size={25} className="text-gray-600 cursor-pointer hover:text-blue-500 transition-colors" onClick={handleBackClick} />
              <span className="text-sm font-medium text-gray-600">Back</span>
            </div>
            {searchUser.map(u => {
              const isOnline = Array.isArray(onlineUser) ? onlineUser.includes(u._id) : false;
              return (
                <div key={u._id} onClick={() => handleSelectChat(u)} className="flex items-center gap-3 p-3 hover:bg-gray-100 cursor-pointer transition-all border-b border-gray-200 last:border-b-0">
                  <div className="relative">
                    <img src={u?.profilepic?.startsWith("/") ? `https://chatify-backend-ybm4.onrender.com${u.profilepic}` : u?.profilepic || "/default-avatar.png"} alt={u.fullname} className="w-10 h-10 rounded-full object-cover" />
        ...
        ) : chatWithMessages.length ? (
          <div>
            <p className="px-3 py-2 text-sm font-semibold text-gray-500">Recent Chatters</p>
            {chatWithMessages.map(c => {
              const isOnline = Array.isArray(onlineUser) ? onlineUser.includes(c._id) : false;
              return (
                <div key={c.conversationId} onClick={() => handleSelectChat(c)} className="flex items-center gap-3 p-3 hover:bg-gray-100 cursor-pointer transition-all border-b border-gray-300 last:border-b-0">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-gray-500 truncate">
                      Last message: {new Date(c.lastUpdated).toLocaleDateString()} , {new Date(c.lastUpdated).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                    </span>
                    {unreadCounts[c.conversationId] > 0 && <span className="ml-auto bg-blue-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{unreadCounts[c.conversationId]}</span>}
                  </div>
                </div>
              );
            })}
