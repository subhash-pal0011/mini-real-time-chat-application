...
const LeftContainer = ({ closeSidebar }) => {
  ...
  const handleBackClick = () => {
    setSearchUser([]);
    setValue("text", "");
    if (closeSidebar) closeSidebar(); // mobile pe sidebar close
  };
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-b from-white to-gray-50 border-r border-gray-200 shadow-lg">
      {/* HEADER */}
      <div className="border-b border-gray-200 bg-white/70 backdrop-blur-md flex justify-between px-6 items-center sticky top-0 z-10 py-3 md:hidden">
        <h2 className="text-xl font-semibold text-gray-800 tracking-tight">Chats</h2>
        <button onClick={handleBackClick} className="text-gray-500 hover:text-gray-700 text-lg">
          ✕
        </button>
      </div>
