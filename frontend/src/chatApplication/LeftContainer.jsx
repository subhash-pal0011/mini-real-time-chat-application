...
const LeftContainer = ({ closeSidebar }) => {
  const { register, watch, setValue } = useForm();
  const searchValue = watch("text", "");
  const [loading, setLoading] = useState(false);
  const [searchUser, setSearchUser] = useState([]);
  ...
  const fetchSearchUsers = async (query) => {
    ...
    try {
      ...
      setSearchUser(res.data.success ? res.data.data : []);
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-b from-white to-gray-50 border-r border-gray-200 shadow-lg">
      ...
      <div className="border-b border-gray-200 bg-white/70 backdrop-blur-md flex justify-between px-6 items-center sticky top-0 z-10 py-3">
        ...
        <motion.img
          src={user?.profilepic?.startsWith("/") ? `https://chatify-z6db.onrender.com${user.profilepic}` : user?.profilepic || "/default-avatar.png"}
          alt="profile"
          className="w-11 h-11 rounded-full object-cover cursor-pointer"
            ...
            {searchUser.map((u) => {
              ...
              return (
                <div key={u._id} onClick={() => handleSelectChat(u)} className="flex items-center gap-3 p-3 hover:bg-gray-100 cursor-pointer transition-all border-b border-gray-200 last:border-b-0">
                  <div className="relative">
                    <img
                      src={u?.profilepic?.startsWith("/") ? `https://chatify-z6db.onrender.com${u.profilepic}` : u?.profilepic || "/default-avatar.png"}
                      alt={u.fullname}
                      className="w-10 h-10 rounded-full object-cover"
                    />
            ...
            {chatWithMessages.map((c) => {
              ...
              return (
                <div key={c.conversationId} onClick={() => handleSelectChat(c)} className="flex items-center gap-3 p-3 hover:bg-gray-100 cursor-pointer transition-all border-b border-gray-300 last:border-b-0">
                  <div className="relative">
                    <img src={c?.profilepic?.startsWith("/") ? `https://chatify-z6db.onrender.com${c.profilepic}` : c?.profilepic || "/default-avatar.png"} alt={c.fullname} className="w-10 h-10 rounded-full object-cover" />
