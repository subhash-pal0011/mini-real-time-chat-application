...
const RightContainer = () => {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        const res = await axios.get(`/api/getmessage/${selectedConversation._id}`);
        if (res.data.success && Array.isArray(res.data.messages)) setMessages(res.data.messages);
        else setMessages([]);
      } catch (e) {
        console.error("Message fetch error:", e.response?.data?.message || e.message);
        setMessages([]);
  ...
  // Listen for realtime socket messages
  useEffect(() => {
    if (!socket) return;
    ...
    const handleReceiveMessage = (newMsg) => {
      ...
      if (msgConvId === activeConvId) addMessage(newMsg);
      else incrementUnread(msgConvId);
    };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await axios.post(`/api/postmessage/${selectedConversation._id}`, { message: data.text.trim() });
      if (res.data.success) {
        const newMsg = res.data.data;
        GlobellyMessage.getState().addMessage(newMsg);
        socket?.emit("sendMessage", { to: selectedConversation._id, from: currentUserId, ...newMsg });
        setValue("text", "");
      }
    } catch (e) {
  ...
  useEffect(() => {
    if (messagesEndRef.current) {
      const parent = messagesEndRef.current.parentNode;
      parent.scrollTo({ top: parent.scrollHeight - 80, behavior: "smooth" });
    }
  }, [messages]);
  ...
  return (
    <div className="flex flex-col h-full w-full bg-gradient-to-br from-gray-50 to-gray-100 shadow-lg rounded-xl overflow-hidden">
      {selectedConversation ? (
        <div className="flex items-center gap-3 p-2 bg-white shadow-md border-b flex-shrink-0">
          <img
            src={selectedConversation?.profilepic?.startsWith("/") ? `https://chatify-backend-ybm4.onrender.com${selectedConversation.profilepic}` : selectedConversation?.profilepic || "/default-avatar.png"}
            alt={selectedConversation?.fullname || "User"}
            className="w-12 h-12 rounded-full object-cover"
          />
          <div>
            <h2 className="text-lg font-semibold text-gray-800">{selectedConversation?.fullname || "Unknown User"}</h2>
        ...
        {Array.isArray(messages) && messages.length > 0 ? messages.map((msg, index) => {
          ...
          return (
            <div key={msg?._id || `temp-${index}`} className={`flex ${isSender ? "justify-end" : "justify-start"}`}>
              <div className={`relative max-w-[45%] px-2 py-1 rounded-2xl text-sm shadow-lg transform transition-all duration-200 hover:scale-[1.02] ${isSender ? "bg-gradient-to-br from-purple-500 to-purple-700 text-white rounded-br-none border border-gray-200" : "bg-gradient-to-br from-gray-50 to-gray-200 text-gray-800 rounded-bl-none border border-gray-200"}`}>
                <p className="whitespace-pre-wrap leading-relaxed font-medium break-words break-all">{msg?.message}</p>
                <span className={`text-[11px] mt-1 block text-right ${isSender ? "text-purple-100" : "text-gray-500"}`}>
      ...
      {/* Input */}
      {selectedConversation && (
        <form onSubmit={handleSubmit(onSubmit)} className="flex items-end bg-white border border-gray-300 rounded-full px-3 py-1 shadow-sm mt-2 mx-4 mb-3">
          <textarea
            placeholder="Enter your message..."
            {...register("text")}
            rows={1}
            ...
            className="outline-none w-full resize-none bg-transparent text-base max-h-32 overflow-y-auto mb-1.5 cursor-pointer"
          />
          <button type="submit" className="ml-2 p-2 rounded-full text-blue-500 hover:text-blue-600 hover:bg-blue-100 transition disabled:opacity-50 cursor-pointer" disabled={isSubmitting || !watchedText?.trim()}>
            {isSubmitting ? <div className="w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" /> : <MdSend className="text-xl" />}
          </button>
        </form>
      )}
    </div>
