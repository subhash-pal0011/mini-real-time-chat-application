...
const Signup = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // Backend se profile pic path handle
      const profilePicUrl = result.user.profilepic.startsWith("/uploads")
        ? `https://chatify-z6db.onrender.com${result.user.profilepic}`
        : result.user.profilepic;

      // Save in localStorage
      localStorage.setItem("chatApplication", JSON.stringify({
        ...result.user,
